...
func NewBlockStore(db db.DB) *BlockStore {
...
func (bs *BlockStore) LoadBlock(height int64) *types.Block {
	...
	total := blockMeta.BlockID.PartSetHeader.Total

	parts, err := bs.db.GetBlockParts(int(height), int(total)) // <<<<<<<<<<<<<<<<<<
	if err != nil {
		panic(err)
	}
	buf := make([]byte, 0, types.BlockPartSizeBytes*total)

	for _, bz := range parts {
...
func (bs *BlockStore) LoadBlockPart(height int64, index int) *types.Part {
	...
	bz, err := bs.db.GetBlockPart(int(height), index) // <<<<<<<<<<<<<<<<<<<
	if err != nil {
		panic(err)
	}
	if len(bz) == 0 {
...
func (bs *BlockStore) saveBlockPart(height int64, index int, part *types.Part) {
	...
	bs.db.SetBlockPart(calcBlockPartKey(height, index), partBytes)
}
...
func (bs *BlockStore) SaveProvData(prov *tmproto.Prov) error {
	pb, err := proto.Marshal(prov)
	if err != nil {
		return err
	}
	bs.db.SetProvData([]byte(prov.Tx.TxBody.EntityId), pb)
	return nil
}

func (bs *BlockStore) LoadProvData(entityId string) (*tmproto.ProvDataList, error) {
	values, err := bs.db.GetProvData(entityId)
	if err != nil {
		return nil, err
	}

	pdl := new(tmproto.ProvDataList)
	pdl.ProvDataList = make([]*tmproto.Prov, 0, len(values))

	for _, value := range values {
		var provData tmproto.Prov
		err = proto.Unmarshal(value, &provData)
		if err != nil {
			return nil, err
		}

		pdl.ProvDataList = append(pdl.ProvDataList, &provData)
	}
