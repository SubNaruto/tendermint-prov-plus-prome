...
func (c *baseRPCClient) ProvQuery(ctx context.Context, entityId string) (*ctypes.ResultProv, error) {
	result := new(ctypes.ResultProv)
	params := make(map[string]interface{})
	params["entityId"] = entityId
	_, err := c.caller.Call(ctx, "prov_query", params, result)
	if err != nil {
		return nil, err
	}
	return result, nil
