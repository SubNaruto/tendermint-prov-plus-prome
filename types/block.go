...
func (b *Block) ValidateBasic() error {
	...
	if !bytes.Equal(b.DataHash, hash) {
		return fmt.Errorf(
...
func (b *Block) fillHeader() {
	...
	if b.DataHash == nil {
		b.DataHash, b.MBTreeRoot = b.Data.Hash()
	}
...
type Header struct {
	...
	MBTreeRoot *tmproto.Node `json:"m_b_tree_root,omitempty"`
}
...
func (data *Data) Hash() (tmbytes.HexBytes, *tmproto.Node) {
	var mbtree *tmproto.Node
	if data == nil {
		return (Txs{}).Hash()
	}
	if data.hash == nil {
		data.hash, mbtree = data.Txs.Hash() // NOTE: leaves of merkle tree are TxIDs
	}
	return data.hash, mbtree
}
