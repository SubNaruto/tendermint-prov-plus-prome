...
func (cs *State) finalizeCommit(height int64) {
	...
	if cs.blockStore.Height() < block.Height {
		...
		go func() {
			// 生成每个交易的溯源数据
			for _, pbtx := range block.Txs {
				var tx tmproto.Tx
				err := proto.Unmarshal(pbtx, &tx)
				if err != nil {
					panic(err)
				}

				// 构造VO
				vo, err := types.CreateVO(block.MBTreeRoot, []byte(tx.Key))
				if err != nil {
					panic(err)
				}
				...
				prov := &tmproto.Prov{
					Tx:         &tx,                   // 交易信息
					Height:     block.Height,          // 交易所在的高度
					MBTreeRoot: block.MBTreeRoot.Hash, // MB树根哈希
					VO:         vo,                    // 交易的VO
				}

				// 将溯源数据存入DB
				err = cs.blockStore.SaveProvData(prov)
				if err != nil {
					panic(err)
				}
			}
